...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['accion'] === 'agregar') {
    ...
    header("Location: exploradores.php?ok=1");
    exit;
}
// ---------- Editar / Eliminar ----------//
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['accion'] === 'editar') {
        ...
        header('Location: exploradores.php?ok=1'); exit;
    }
    if ($_POST['accion'] === 'eliminar') {
        eliminar_muchacho($_POST['id']);
        header('Location: exploradores.php?eliminado=1'); exit;
    }
}
//----------------------------------------------------------------//

// Consulta exploradores (Departamento 5)
$consultaGEN = "SELECT m.*, d.ID_DEPARTAMENTO, d.NOMBRE_DEP
                FROM muchachos m
                JOIN departamento d ON m.DEPARTAMENTO = d.ID_DEPARTAMENTO
                WHERE m.DEPARTAMENTO = '9'";
...
<section class="bg-dark text-light text-center p-5">
  <div class="container" id="Tablaexploradores">
    <h1 class="my-5"><span style="color:rgba(255,255,0,.9)">Registro de Exploradores</span></h1>
...
(() => {
  ...
  const tbody       = document.querySelector('#Tablaexploradores tbody');
